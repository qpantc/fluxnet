...
flux_guy <- read.csv('/Users/quan/projects/Trait_Orchidee/Flux/Fluxdata/FLX_GF-Guy_FLUXNET2015_FULLSET_2004-2020_Evergreen_Broadleaf_Forests/FLX_GF-Guy_FLUXNET2015_FULLSET_YY_2004-2020_beta-3.csv')
flux_Bra <- read.csv('/Users/quan/projects/Trait_Orchidee/Flux/Fluxdata/FLX_BE-Bra_FLUXNET2015_FULLSET_1996-2020_Evergreen_Needleleaf_Forests/FLX_BE-Bra_FLUXNET2015_FULLSET_MM_1996-2020_beta-3.csv')
...
vars_long <- c("TIMESTAMP","TA_F","TA_F_NIGHT","TA_F_DAY","SW_IN_F","LW_IN_F",
               "VPD_F","P_F","WS_F","SWC_F_MDS_1",
               'NETRAD','TS_F_MDS_1','G_F_MDS','LE_F_MDS','H_F_MDS',
...
## Step 1
# Filter weird values
vars <- c("T","T_NIGHT","T_DAY","SWDOWN","LWDOWN" ,"VPD","RAIN","WIND","SWC","RN","TSOIL","GF","QLE","QH","NEE","RECO","GPP") 
for (var in vars){
...
```{r}
# Min and max values
flux_selected.converted %>% dplyr::select(-c("Time","year","month")) %>% summarise_all(min, na.rm = TRUE)
flux_selected.converted %>% dplyr::select(-c("Time","year","month")) %>% summarise_all(max, na.rm = TRUE)
```

- Calculate annual sum of precipitation, and the annual mean for each other variable. 
```{r}
# Sum for Precip, mean for the rest
flux_selected.converted %>% dplyr::select(c("year","RAIN","month")) %>% group_by(year) %>% summarise(Precip = sum(RAIN, na.rm = TRUE))
flux_selected.converted %>% dplyr::select(-c("RAIN","Time","month")) %>% group_by(year) %>% summarise_all(mean, na.rm = TRUE)

flux_selected.converted %>% dplyr::select(c("year","RAIN","month")) %>% group_by(year) %>% summarise(Precip = sum(RAIN)) %>% pull(Precip) %>% mean(na.rm = TRUE)
flux_selected.converted %>% dplyr::select(-c("RAIN","Time","month","year")) %>% summarise_all(mean, na.rm = TRUE)

```


### Step 2: annual flux dynamics
Seasonal cycle, patterns, extreme events 

```{r}
flux_selected.cycle <- flux_selected.converted %>% dplyr::select(-c("month","Time")) %>% group_by(year) %>% summarise_all(mean,na.rm = TRUE)
flux_selected.cycle.long <- flux_selected.cycle %>% pivot_longer(cols = -c("year"),
                                               names_to = "var",
                                               values_to = "value")

flux_selected.cycle.long.years <- flux_selected.converted %>% group_by(Time) %>% 
  summarise_all(mean,na.rm = TRUE) %>% pivot_longer(cols = -c("Time"),
                                                   names_to = "var",
                                                   values_to = "value")
```
...
ggplot(data = flux_selected.cycle.long %>% 
  ...
  geom_line(aes(x = year,y = value)) +
    # geom_smooth(aes(x = year,y = value),colour="red", size=1, method="loess",
    #           span=1/5, se=FALSE) +
  facet_wrap(~var,scales = "free") +

  theme_bw()

ggplot(data = flux_selected.cycle.long.years %>% 
         dplyr::filter(var %in%  c("T_DAY","VPD","RAIN","WIND","SWC","GPP"))) +
  geom_line(aes(x = Time,y = value)) +
    # geom_smooth(aes(x = Time,y = value),colour="red", size=1, method="loess",
    #           span=1/5, se=FALSE) +
  facet_wrap(~var,scales = "free",ncol = 2) +
  theme_bw()

ggplot(data = flux_selected.cycle.long.years %>% 
         dplyr::filter(var %in%  c("NEE","RECO","GPP"))) +
  geom_line(aes(x = Time,y = value)) +
    # geom_smooth(aes(x = Time,y = value),colour="red", size=1, method="loess",
    #           span=1/5, se=FALSE) +
  facet_wrap(~var,scales = "free",ncol = 2) +
